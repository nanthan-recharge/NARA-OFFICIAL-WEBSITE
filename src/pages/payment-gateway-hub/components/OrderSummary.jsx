    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-6">
      ...
      <div className="flex items-center justify-between pb-4 border-b border-gray-200">
        <div>
          ...
          {orderNumber && (
            <p className="text-sm text-gray-500 mt-1">{t('orderSummary.orderNumber', { orderNumber })}</p>
          )}
        </div>
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm text-gray-600">{t('orderSummary.customerType')}</span>
      ...
      <div className="space-y-4">
        <h3 className="font-medium text-gray-900">{t('orderSummary.items', { count: items?.length })}</h3>
                  ...
                  <div className="text-right">
                    ...
                    {item?.quantity > 1 && (
                      <div className="text-xs text-gray-500">
                        {t('orderSummary.lineTotal', { amount: formatPrice(item?.price * item?.quantity) })}
                      </div>
                    )}
                  </div>
                </div>
      ...
      <div className="space-y-3 pt-4 border-t border-gray-200">
        ...
        <div className="flex justify-between text-sm items-center">
          <div className="flex items-center gap-1">
            <span className="text-gray-600">{t('orderSummary.processingFee')}</span>
            <div className="group relative">
              ...
              <div className="invisible group-hover:visible absolute bottom-full left-0 mb-2 px-3 py-2 bg-gray-900 text-white text-xs rounded-lg whitespace-nowrap z-10">
                {t('orderSummary.processingFeeInfo')}
              </div>
            </div>
          </div>
        ...
        {taxes > 0 && (
          <div className="flex justify-between text-sm">
            <span className="text-gray-600">{t('orderSummary.vat')}</span>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="text-sm">
            <div className="font-medium text-blue-900 mb-1">{t('orderSummary.securePaymentGuarantee')}</div>
            <div className="text-blue-700">
              {t('orderSummary.securePaymentText')}
            </div>
          </div>
        </div>
      </div>
      ...
      {customerType === 'government' && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3">
          <div className="flex items-center gap-2 text-sm text-green-800">
            ...
            <span className="font-medium">{t('orderSummary.notices.government.title')}</span>
          </div>
          <p className="text-xs text-green-700 mt-1">
            {t('orderSummary.notices.government.description')}
          </p>
        </div>
      )}
      {customerType === 'institutional' && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
          <div className="flex items-center gap-2 text-sm text-blue-800">
            ...
            <span className="font-medium">{t('orderSummary.notices.institutional.title')}</span>
          </div>
          <p className="text-xs text-blue-700 mt-1">
            {t('orderSummary.notices.institutional.description')}
          </p>
        </div>
      )}
